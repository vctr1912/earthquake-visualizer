...
const App = () => {
 ...
 return (
    <div className="relative w-full h-screen">
      {/* ---- App Logo + Title ---- */}
      <div 
        className={`absolute top-4 left-4 flex items-center justify-center gap-2 font-bold text-xl sm:text-2xl sm:px-4 py-1 z-20 
          ${activeLayer.id === 'openstreetmap' ? 'text-gray-700' : 'text-white'}`}
        style={{ transform: 'none' }} // Prevent transform inheritance
      >
        <img 
          src="logo.png" 
          alt="Logo" 
      ...
      {/* ---- Loader Overlay ---- */}
      {showLoading && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/5 backdrop-blur-xs z-30" style={{ transform: 'none' }}>
      ...
      {/* ---- Error Message ---- */}
      {error && !showLoading && (
        <div className="absolute bottom-4 left-4 z-20 text-red-600 bg-slate-50 border border-gray-300 rounded-lg px-4 py-2 shadow-lg" style={{ transform: 'none' }}>
          {error}
        </div>
      )}
